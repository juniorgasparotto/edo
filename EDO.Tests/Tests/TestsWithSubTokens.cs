...
{
    [TestClass]
    public class TestsWithSubTokens
    {
        ...
        public void TestHierarchy()
        {
            ...
            var converterToDebug = new TokenToDebug(false, delimiterMainTokens);
            var debug = converterToDebug.Convert(tokenCollection);

            var converterHierarchy = new TokenToHierarchy(false, delimiterMainTokens);
        ...
        public void UpdateByCode()
        {
            ...
            foreach (var test in tests)
            {
                ...
                test.OutputNormal = (new TokenToExpression(false, delimiterMainTokens)).Convert(convertToToken.Convert(collection));

                var convertToToken2 = new EdoObjectToToken(TokenizeType.AwaysRepeatDefinedToken);
                test.OutputAwaysRepeatDefinedToken = (new TokenToExpression(false, delimiterMainTokens)).Convert(convertToToken2.Convert(collection));

                var convertToToken3 = new EdoObjectToToken(TokenizeType.NeverRepeatDefinedTokenIfAlreadyParsed);
                test.OutputNeverRepeatDefinedTokenIfAlreadyParsed = (new TokenToExpression(false, delimiterMainTokens)).Convert(convertToToken3.Convert(collection));

                test.OutputHierarchyNormal = new TokenToHierarchy(false, delimiterMainTokens).Convert(new EdoObjectToToken(TokenizeType.Normal).Convert(collection));
                test.OutputHierarchyAwaysRepeatDefinedToken = new TokenToHierarchy(false, delimiterMainTokens).Convert(new EdoObjectToToken(TokenizeType.AwaysRepeatDefinedToken).Convert(collection));
                test.OutputHierarchyNeverRepeatDefinedTokenIfAlreadyParsed = (new TokenToHierarchy(false, delimiterMainTokens)).Convert(new EdoObjectToToken(TokenizeType.NeverRepeatDefinedTokenIfAlreadyParsed).Convert(collection));
                ...
                test.OutputDebugNormal = new TokenToDebug(false, delimiterMainTokens).Convert(new EdoObjectToToken(TokenizeType.Normal).Convert(collection));
                test.OutputDebugAwaysRepeatDefinedToken = new TokenToDebug(false, delimiterMainTokens).Convert(new EdoObjectToToken(TokenizeType.AwaysRepeatDefinedToken).Convert(collection));
                test.OutputDebugNeverRepeatDefinedTokenIfAlreadyParsed = new TokenToDebug(false, delimiterMainTokens).Convert(new EdoObjectToToken(TokenizeType.NeverRepeatDefinedTokenIfAlreadyParsed).Convert(collection));
        ...
        public void ValidateCenaries(TestExpression test, EDObjectCollection collection)
        {
            ...
            var normal = (new TokenToExpression(false, delimiterMainTokens)).Convert(tokensNormal);
            ...
            var awaysRepeat = (new TokenToExpression(false, delimiterMainTokens)).Convert(tokensAwaysRepeat);
            ...
            var neverRepeat = (new TokenToExpression(false, delimiterMainTokens)).Convert(tokensNeverRepeat);
        ...
        private void ValidateDebug(TestExpression test, TokenGroupCollection collection, string output)
        {
            ...
            var convert = new TokenToDebug(false, delimiterMainTokens, delimiterDebugRef);
